  ...
  return (
    <section className="s s-taka" data-screen-label="02 Taka Note">
      <div className="wrap">
            ...
            {TAKA_SECTORS.map((s, idx) => {
              ...
              return (
                <div
                  ...
                  onKeyDown={(e) => {
                    if (e.key === "Enter" || e.key === " ") { e.preventDefault(); selectSector(s.key, true); }
                    else if (e.key === "ArrowRight" || e.key === "ArrowLeft") {
                      e.preventDefault();
                      const ni = (idx + (e.key === "ArrowRight" ? 1 : -1) + TAKA_SECTORS.length) % TAKA_SECTORS.length;
                      selectSector(TAKA_SECTORS[ni].key, false);
                      const sib = e.currentTarget.parentNode.children[ni];
        ...
        <RelevantNews items={RELEVANT_NEWS.taka} accent="#0185C6" />
      </div>
    </section>
  );
}
...
function GDPSection() {
  ...
  useEffect(() => {
    if (!inView) return;
    if (prefersReducedMotion()) { setDraw(1); return; }
    let raf, t0 = null;
    const dur = 1800;
    const tick = (t) => {
      if (t0 === null) t0 = t;
      ...
      <div className="wrap">
        <div className="section-head">
          ...
          <p className="lede" style={{ marginTop: 18 }}>
           Despite rising in nominal terms, Bangladesh's total budget has steadily shrunk as a share of GDP, falling from a peak of <b>16.8%</b> in <b>FY13</b> to <b>13.7%</b> in <b>FY27</b>, suggesting the state is spending less relative to the size of the economy it governs. A shrinking budget ratio means less room for public investment in health, education, and infrastructure, <b><i>even as the economy grows</i></b>. 
          </p>
        </div>
        ...
        <RelevantNews items={RELEVANT_NEWS.gdp} accent="#45B7D1" />
      </div>
    </section>
  );
}
